...
public class Main_Game_Camera : MonoBehaviour {
	...
	void Update () {
		...
		if(Input.GetKey(KeyCode.A) && Camera.main.orthographicSize <= Manager.CAM_MAX_DIST)
			Camera.main.orthographicSize += Manager.CAM_MOVE_SPEED;
		if(Input.GetKey(KeyCode.S) && Camera.main.orthographicSize >= Manager.CAM_MIN_DIST)
			Camera.main.orthographicSize -= Manager.CAM_MOVE_SPEED;
	}
}
